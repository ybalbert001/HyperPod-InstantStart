...
--trust-remote-code`;
  ...
  const validateVllmCommand = (_, value) => {
    ...
    const isValidCommand = supportedCommands.some(cmd => 
      ...
      <Form.Item
        label={
          <Space>
            <CodeOutlined />
            VLLM/SGLang Command
            <Tooltip title="Complete command to run the model server. The system will automatically extract model information from this command.">
              <InfoCircleOutlined />
            </Tooltip>
          </Space>
        }
